...
// Note: Frontend now uses Direct WebSocket for speed, this remains as a legacy/fallback route
...
// 🗄️ Database Connection String (Enhanced logging)
...
// process.exit(1); // Kept running to allow partial functionality
});
...
// 🔥 CRITICAL FIX: Delete Webhook before starting to prevent 409 Conflict
 ...
 bot.on('message', async (ctx) => {
     ...
     if (text.startsWith('/')) {
         ...
         if (code) {
             try {
                 // 🔒 SANDBOX EXECUTION (SAFE CONTEXT)
                 // Wraps code in a safe function scope
...
🔹 ROUTE: AI Generation (Legacy / Backup)

The Frontend now uses Direct WebSocket to 'backend.buildpicoapps.com' for faster streaming.

This route is kept as a robust fallback in case frontend connection fails.
*/
